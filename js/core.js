...
s.packaging.forEach((v,i)=>{
  ...
  v.suppliers=v.suppliers.map((x,j)=>{
    ...
    return{...x,id:x.id||crypto.randomUUID(),name:x.name||'Lieferant '+(j+1),url:x.url||'',priceType,price,
      ...
      totalShipping:num(x.totalShipping),imageUrl:x.imageUrl||'',paymentFeePct:Math.max(0,num(x.paymentFeePct,0)),paymentFee:Math.max(0,num(x.paymentFee,0)),vatRate:Math.max(0,num(x.vatRate,0)),vatIncluded:!!x.vatIncluded,customs:!!x.customs,preferred:!!x.preferred,priceTiers:Array.isArray(x.priceTiers)?x.priceTiers.map(t=>{
  ...
  return migrateAlibabaTaxDefaultsOnce(migrateAlibabaCustomsOnce(s));
}

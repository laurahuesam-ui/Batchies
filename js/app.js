...
['batchTargetMargin','batchSalePrice','batchUseOffsite','batchUseCurrency','batchUseSetup','batchLaborMinutes','batchHourlyRate','batchOutboundShipping','batchAdCost','batchRiskPct','batchFixedAllocation'].forEach(id=>$('#'+id).addEventListener('input',liveBatchCalc));$('#batchUseRecommendedBtn').onclick=()=>{$('#batchSalePrice').value=batchCalc(collectBatchDraft()).recommended.toFixed(2);liveBatchCalc()};

...
initColorFilter();$('#addProductBtn').onclick=()=>openProduct();$('#addPackagingBtn').onclick=()=>openPackaging();$('#addBatchBtn').onclick=()=>openBatch();
...
});$('#searchProducts').oninput=renderProducts;$('#filterProductColor').onchange=renderProducts;$('#searchPackaging').oninput=renderPackaging;$('#searchBatches').oninput=renderBatches;const refreshSuggestions=$('#refreshBatchSuggestionsBtn');if(refreshSuggestions)refreshSuggestions.onclick=renderBatchSuggestions;
$('#addSupplierBtn').onclick=()=>{const has=$$('.supplier-row').length;$('#supplierRows').insertAdjacentHTML('beforeend',supplierRowHtml({id:crypto.randomUUID(),name:'',url:'',priceType:'unit',price:0,minOrderQty:1,setPrice:0,setQty:1,totalShipping:0,imageUrl:'',customs:false,preferred:!has}));bindSupplierEvents();liveCalc()};

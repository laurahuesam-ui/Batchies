...
$('#addSupplierBtn').onclick=()=>{const has=$$('.supplier-row').length;$('#supplierRows').insertAdjacentHTML('beforeend',supplierRowHtml({id:crypto.randomUUID(),name:'',url:'',priceType:'unit',price:0,minOrderQty:1,setPrice:0,setQty:1,purchasePrice:0,packageCount:1,amountPerPackage:1,consumptionUnit:'m',totalShipping:0,imageUrl:'',customs:false,preferred:!has,priceTiers:[],shippingPoints:[]}));const nr=$$('.supplier-row').at(-1);if(nr)renderSupplierTierData(nr,{priceTiers:[],shippingPoints:[]});bindSupplierEvents();liveCalc()};

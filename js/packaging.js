...
function packagingSupplierRowHtml(s){
  ...
  return `<div class="packaging-supplier-row" data-id="${esc(s.id||crypto.randomUUID())}">
    <div class="supplier-grid packaging-supplier-grid">
      ...
      <div class="supplier-cell"><span class="supplier-mini-label">Zahlungsgebühr (%)</span><input class="packaging-supplier-payment-fee-pct" type="number" min="0" step="0.1" value="${Math.max(0,num(s.paymentFeePct,0))}" placeholder="0" title="Prozentuale Zahlungsabwicklungsgebühr. Sie wird auf den Betrag nach MwSt. berechnet; bei Alibaba aktuell z. B. 3 %."></div>
...
function packagingSupplierFromRow(r,i){
  ...
  return {id:r.dataset.id||crypto.randomUUID(),name:r.querySelector('.packaging-supplier-name').value.trim()||'Lieferant '+(i+1),
    ...
    paymentFeePct:Math.max(0,num(r.querySelector('.packaging-supplier-payment-fee-pct')?.value,0)),

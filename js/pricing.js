...
function supplierCustomsCost(s){return s?.customs?supplierBaseOrderCost(s)*0.12:0}
...
function productInboundShipping(p){const s=(p.suppliers||[]).find(x=>x.preferred)||(p.suppliers||[])[0];if(!s)return 0;const ship=supplierUnitShipping(s),customs=s.customs?(num(p.basePrice)+ship)*0.12:0;return ship+customs}

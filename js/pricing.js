...
function batchCalc(b,overridePrice=null){
  ...
  (b.packagingItems||[]).forEach(i=>{const v=state.packaging.find(x=>x.vid===i.vid),s=(v?.suppliers||[]).find(x=>x.preferred)||(v?.suppliers||[])[0];if(s)packagingCost+=supplierLandedUnitCost(s)*Math.max(1,num(i.qty,1))});
  const costs=productCost+packagingCost,price=overridePrice===null?num(b.salePrice):num(overridePrice),revenue=price;
  ...
  return{productCost,packagingCost,extra:packagingCost,total:costs,costs,price,revenue,fees,profit,margin,recommended}
}

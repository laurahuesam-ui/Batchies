...
function priceTierRowHtml(t={}){const unit=num(t.unitPrice);return `<div class="supplier-price-tier-row"><input class="tier-min" type="number" min="1" step="1" value="${Math.max(1,num(t.minQty,1))}" placeholder="Ab Menge"><input class="tier-max" type="number" min="1" step="1" value="${num(t.maxQty)||''}" placeholder="Bis (leer = offen)"><input class="tier-price" type="number" min="0" step="0.0001" value="${unit||''}" placeholder="€/Stück"><input class="tier-unit-display" type="text" value="${unit?euro(unit):'–'}" readonly title="Preis/Stück dieser Staffel"><button type="button" class="iconbtn remove-price-tier">✕</button></div>`}
...
function renderSupplierTierData(r,s={}){r.querySelector('.supplier-price-tiers').innerHTML=(s.priceTiers||[]).map(priceTierRowHtml).join('');r.querySelector('.supplier-shipping-points').innerHTML=(s.shippingPoints||[]).map(shippingPointRowHtml).join('');updateSupplierTierSummary(r);updateSupplierTierUnitDisplays(r);syncSupplierMainFieldsFromTiers(r)}
...
function updateSupplierTierUnitDisplays(r){
  ...
  // Preisstaffeln: direkter Staffelpreis pro Stück.
  r.querySelectorAll('.supplier-price-tier-row').forEach(row=>{
    const price=num(row.querySelector('.tier-price')?.value),
      out=row.querySelector('.tier-unit-display');
    if(out)out.value=price>0?euro(price):'–'
  });

  // Versandpunkte: passende Staffel + tatsächlicher Versand, geteilt durch Menge.
  const tiers=collectSupplierPriceTiers(r);
  r.querySelectorAll('.supplier-shipping-point-row').forEach(row=>{
    const qty=Math.max(1,num(row.querySelector('.ship-point-qty')?.value,1)),
    ...
    const tier=tiers.find(t=>qty>=Math.max(1,num(t.minQty,1))&&(!num(t.maxQty)||qty<=num(t.maxQty)));
    const fallbackType=r.querySelector('.supplier-price-type')?.value==='set'?'set':'unit';
    const fallbackEntry=num(r.querySelector('.supplier-price-entry')?.value);
    const fallbackQty=Math.max(1,num(r.querySelector('.supplier-qty')?.value,1));
    const unitPrice=tier&&num(tier.unitPrice)>0
      ? num(tier.unitPrice)
      : (fallbackType==='set'?fallbackEntry/fallbackQty:fallbackEntry);

    const shipping=withCustoms>0?withCustoms:normal;
    let total=unitPrice*qty+shipping;

    // Nur wenn KEIN Versand inkl. Zollabwicklung eingetragen wurde,
    // gilt der normale manuelle 12%-Zollpuffer.
    if(withCustoms<=0 && r.querySelector('.supplier-customs')?.checked){
      total*=1.12;
    }

...
function updateSupplierDerived(r){
  if(!r)return;
  const type=r.querySelector('.supplier-price-type').value==='set'?'set':'unit',
    s=supplierDraftForDerived(r),
    active=supplierActiveCalcSource(s),
    qty=supplierCalcQty(s),
    order=supplierOrderCost(s),
    unit=qty>0?order/qty:0;

  const pl=r.querySelector('.supplier-price-label');
  if(pl)pl.textContent=type==='set'?'Setpreis':'Stückpreis';
  const ql=r.querySelector('.supplier-qty-label');
  if(ql)ql.textContent=type==='set'?'Stück im Set':'Mindestbestellmenge';
  r.querySelector('.supplier-qty').placeholder=type==='set'?'Stück im Set':'MOQ';

  const uc=r.querySelector('.supplier-unit-cost');
  if(uc){
    uc.value=euro(unit);
    uc.title=`Aktive Kalkulation: ${active.type==='tier'?'Preisstaffel ab '+active.qty:active.type==='shipping'?'Versandpunkt '+active.qty:'Lieferantenzeile'}`
  }
  const oc=r.querySelector('.supplier-order-cost');
  if(oc){
    oc.value=euro(order);
    oc.title=`Bestellwert für aktive Kalkulationsmenge ${qty}`
  }
  const summary=r.querySelector('.supplier-tier-summary');
  if(summary){
    const base=summary.dataset.base||summary.textContent||'';
    summary.textContent=`${base.replace(/\s·\sAktiv:.+$/,'')} · Aktiv: ${active.type==='main'?'Lieferantenzeile':active.type==='tier'?'Preisstaffel ab '+active.qty:'Versandpunkt '+active.qty}`
  }
  applySupplierCalcSourceVisual(r)
}

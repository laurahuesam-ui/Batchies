...
function bindBatchPackagingEvents(){$$('.batch-packaging,.batch-packaging-qty').forEach(e=>e.addEventListener('input',liveBatchCalc));$$('.remove-batch-packaging').forEach(b=>b.addEventListener('click',()=>{b.closest('.batch-packaging-row').remove();liveBatchCalc()}))}
